...
export async function aiEncrypt(
  ...
  const strategyByRound = round <= 2
    ? `现在是第${round}轮（早期）。
策略：建立有个性的线索风格，大胆发挥想象力。
- 鼓励使用故事性、画面感强的短语，不要局限于两个字的词
- 可以联想到具体场景、人物、事件、感受、声音、气味等
- 禁止使用同义词、近义词、上位词/下位词
- 线索越"有画面感"越好——队友看到线索就能联想到对应关键词
...
联想维度参考（每条线索选不同维度）：
  ...
  感官体验："咬一口酸掉牙" → 关键词"柠檬"
  文化/典故："嫦娥奔月" → 关键词"兔子"
  反差/幽默："世界上最慢的快递" → 关键词"蜗牛"
  声音/动作："哗啦啦" → 关键词"瀑布"

坏的线索（太无聊/太直接）：
  关键词"苹果" → "水果"（上位词）、"红色"（太直接）、"梨子"（近义联想）`
    : round <= 4
    ? `现在是第${round}轮（中期），对手已通过历史线索开始分析你的关键词。
策略：发挥想象力，用意想不到的角度给线索。
- 每轮必须换一个全新的联想角度，不要和之前任何一轮雷同
- 用具体的、有画面感的短语代替抽象的两字词
- 可以用：反常识联想、冷知识、跨领域类比、感受描述、声音模拟等
- 例如：关键词"月亮"，之前用过"嫦娥"，这轮可以用"潮汐的指挥家"
- 关键原则：队友知道关键词所以能秒懂，对手不知道关键词所以摸不着头脑`
    : `现在是第${round}轮（后期），对手已积累大量历史数据。
策略：最大化想象力，使用高度个人化和跳跃性的联想。
- 用只有知道关键词才能"恍然大悟"的线索
- 可以使用：主观感受、通感、隐喻、反讽、谐音梗、网络用语、跨文化联想
- 例如：关键词"冰箱" → "北极熊的卧室"；关键词"头发" → "烦恼丝"
- 越天马行空越好，但队友必须能从4个关键词中选出正确的那个
- 每条线索必须与之前所有轮次的维度和风格完全不同`;
...
## 线索规则
- 每条线索可以是一个词、一个短语或一个小短句（2-8个字），不要总是只给两个字
- 禁止使用：关键词本身、关键词的一部分、同义词、近义词、上位词/下位词
- 不能重复之前用过的线索
- 核心原则：线索要有想象力、有画面感、有趣味性，让队友看到线索时会心一笑并联想到正确关键词
